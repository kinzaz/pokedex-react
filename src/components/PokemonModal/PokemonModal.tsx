...
import { selectCurrentPokemon } from '@/features/pokemon/model/pokemon-selector';
import { ErrorMessage } from '../PokemonType/styles';
import { useAppDispatch } from '@/store';
import { clearPokemon } from '@/features/pokemon/model/pokemon.slice';
import { setModal } from '@/features/displayedParams/model/displayedParams.slice';

export const PokemonModal = () => {
	const dispatch = useAppDispatch();
	const mobile = useMedia('(max-width: 980px)');

	const { currentPokemon: pokemonData } = useSelector(selectCurrentPokemon);
	...
	if (pokemonData) {
		...
		const renderCloseButton = () => (
			<C.CloseButton onClick={handleClose} mobile={mobile}>
				<CloseIcon />
			</C.CloseButton>
		);
